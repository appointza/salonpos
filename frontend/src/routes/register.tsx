...
import { Building2, Check, ChevronLeft, ChevronRight, Plus, Sparkles, Trash2, Upload, User } from "lucide-react";
...
type RegisterIntent = "customer" | "organization" | null;
...
/** Shown in the org wizard for now. Other steps stay in the file but are hidden. */
const VISIBLE_STEP_INDICES = [0, 2, 8, 9];

function clampVisibleStep(step: number) {
  if (VISIBLE_STEP_INDICES.includes(step)) return step;
  const previous = [...VISIBLE_STEP_INDICES].reverse().find((s) => s <= step);
  return previous ?? VISIBLE_STEP_INDICES[0]!;
}
...
function RegisterPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const intent =
          parsed.intent ??
          (parsed.step && parsed.step > 0 ? "organization" : parsed.admin?.name ? "organization" : null);
        setDraft({
          ...emptyDraft(),
          ...parsed,
          intent,
          step: clampVisibleStep(parsed.step ?? 0),
          customer: { ...emptyDraft().customer, ...parsed.customer },
        });
      }
    } catch {
      /* ignore */
  ...
  const step = clampVisibleStep(draft.step);
  const visiblePos = VISIBLE_STEP_INDICES.indexOf(step);
  const visibleCount = VISIBLE_STEP_INDICES.length;
  const isLastVisible = visiblePos === visibleCount - 1;
  const progress = Math.round(((visiblePos + 1) / visibleCount) * 100);
  ...
  function validate(): string | null {
    if (step === 0) {
      ...
      if (draft.otp.replace(/\D/g, "").length !== 6) return "Enter the 6-digit verification code (demo: any 6 digits).";
    }
    if (step === 2) {
  ...
  function next() {
    ...
    if (step === 0) set("admin", { ...draft.admin, verified: true });
    const nextStep = VISIBLE_STEP_INDICES[visiblePos + 1];
  ...
  function back() {
    if (draft.intent === "customer" || (draft.intent === "organization" && visiblePos <= 0)) {
      set("intent", null);
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
    }
    const prevStep = VISIBLE_STEP_INDICES[visiblePos - 1] ?? 0;
    setDraft((d) => ({ ...d, step: prevStep }));
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        {!draft.intent && (
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm sm:p-8">
            <StepShell
              title="How will you use Luxe Salon?"
              hint="Choose the account that fits you. You can always log in from the same sign-in page later."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <button
                  type="button"
                  onClick={() => set("intent", "customer")}
                  className="rounded-xl border border-border p-6 text-left transition-colors hover:border-primary hover:bg-primary/5"
                >
                  <User className="size-6 text-primary" />
                  <p className="mt-3 font-display text-lg">I'm a customer</p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Create a personal account to find nearby salons and book appointments.
                  </p>
                </button>
                <button
                  type="button"
                  onClick={() => set("intent", "organization")}
                  className="rounded-xl border border-border p-6 text-left transition-colors hover:border-primary hover:bg-primary/5"
                >
                  <Building2 className="size-6 text-primary" />
                  <p className="mt-3 font-display text-lg">I'm a business / organisation</p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Register your salon, studio or spa and set up your workspace.
                  </p>
                </button>
              </div>
            </StepShell>
        ...
        {draft.intent === "customer" && (
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm sm:p-8">
            <StepShell
              title="Create your customer account"
              hint="A short signup — then you can browse salons near you and book instantly."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <Field
                  label="Full name"
                  ...
                  onChange={(v) => set("customer", { ...draft.customer, name: v })}
                />
                <Field
                  label="Mobile number"
                  required
                  value={draft.customer.phone}
                  onChange={(v) => set("customer", { ...draft.customer, phone: v })}
                />
                <Field
                  label="Email address"
                  type="email"
                ...
                <Field
                  ...
                  onChange={(v) => set("customer", { ...draft.customer, password: v })}
                />
                <Field
                  label="Confirm password"
                  required
                  type="password"
                  value={draft.customer.confirm}
                  onChange={(v) => set("customer", { ...draft.customer, confirm: v })}
                />
              </div>
            </StepShell>
        ...
        {draft.intent === "organization" && (
          <>
        <div className="mb-8">
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>
              Step {visiblePos + 1} of {visibleCount} · {STEPS[step]}
            </span>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {VISIBLE_STEP_INDICES.map((i, pos) => (
              <button
                key={STEPS[i]}
                type="button"
                onClick={() => pos < visiblePos && setDraft((d) => ({ ...d, step: i }))}
                className={cn(
                  "rounded-full border px-2.5 py-1 text-[11px] transition-colors",
                  i === step && "border-primary bg-primary text-primary-foreground",
                  pos < visiblePos && "border-border text-muted-foreground hover:bg-accent",
                  pos > visiblePos && "border-dashed border-border text-muted-foreground/60",
                )}
              >
                {pos < visiblePos && <Check className="mr-1 inline size-3" />}
                {STEPS[i]}
              </button>
            ))}
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm sm:p-8">
          {step === 0 && (
            <StepShell title="Create your admin account" hint="You will be the organization ADMIN with full access. Demo mode: enter any 6-digit code to verify.">
              <div className="grid gap-4 sm:grid-cols-2">
          ...
          {step === 2 && (
            <StepShell title="Create your organization" hint="Name your business and claim the public booking URL customers will use.">
              <div className="grid gap-4 sm:grid-cols-2">
