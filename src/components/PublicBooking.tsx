...
import { MapPin, Clock, Scissors, CalendarCheck, ExternalLink } from "lucide-react";
...
export function PublicBooking({ showSwitcher = true }: { showSwitcher?: boolean }) {
  ...
  function book() {
    ...
    setOrgId(org.orgId);
    const customerExists = (allRows["customers"] ?? []).some(
      (c) => String(c["orgId"]) === org.orgId && String(c["phone"]).replace(/\D/g, "") === phone.replace(/\D/g, ""),
    );
    if (!customerExists) {
      create("customers", {
      ...
      <main className="mx-auto max-w-6xl space-y-8 px-4 py-8">
        <section className="rounded-2xl border border-border bg-card p-8 shadow-sm">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            {org.businessType} · Choose a studio near you, pick a service and stylist, and confirm a slot instantly.
            Online bookings sync straight into the salon's POS calendar.
          </p>
        </section>
        ...
        {memberships.length > 0 && (
          <section>
            <h2 className="font-display text-xl">Memberships & offers</h2>
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              {memberships.slice(0, 3).map((m) => (
                <div key={String(m.id)} className="rounded-xl border border-border bg-card p-4">
                  <p className="font-medium">{String(m["plan"])}</p>
