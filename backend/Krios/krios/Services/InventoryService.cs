...
namespace Krios.Services.Krios
{
    public class InventoryService
    {
        ...
        public async Task<List<Inventory>> Select(InventorySelectReq req)
        {
        ...
        public async Task<List<Inventory>> SelectTransaction(IDb db, InventorySelectReq req)
        {
            const string query = @"
                SELECT id, ""orgId"", ""locationId"", name, category, brand, outlet, stock, ""reorderLevel"", ""unitCost"", ""sellPrice"", batch, expiry, ""vendorId"", createdby, createdon, updatedby, updatedon
                FROM inventory
            ";
        ...
        public async Task<Inventory> Insert(Inventory entity)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task InsertTransaction(IDb db, Inventory entity)
        {
            const string query = @"
                INSERT INTO inventory (
                    ""orgId"", ""locationId"", name, category, brand, outlet, stock, ""reorderLevel"", ""unitCost"", ""sellPrice"", batch, expiry, ""vendorId"", createdby, createdon, updatedby, updatedon
                )
                VALUES (
                    @orgId, @locationId, @name, @category, @brand, @outlet, @stock, @reorderLevel, @unitCost, @sellPrice, @batch, @expiry, @vendorId, @createdby, @createdon, @updatedby, @updatedon
                )
        ...
        public async Task<bool> UpdateTransaction(IDb db, Inventory entity)
        {
            const string query = @"
        ...
        public async Task<bool> Delete(InventoryDeleteReq req)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> DeleteTransaction(IDb db, InventoryDeleteReq req)
        {
        ...
        private static Inventory Map(DbDataReader reader)
        {
            return new Inventory
            {
                ...
                stock = ReadLong(reader, "stock"),
                reorderLevel = ReadLong(reader, "reorderLevel"),
                unitCost = ReadDecimal(reader, "unitCost"),
                sellPrice = ReadDecimal(reader, "sellPrice"),
                batch = reader["batch"]?.ToString() ?? "",
                expiry = ReadDate(reader, "expiry"),
                vendorId = ReadLong(reader, "vendorId"),
        ...
        private static void Bind(DbCommand cmd, IDb db, Inventory entity, bool includeId)
        {
            if (includeId)
            ...
            db.AddParameter(cmd, "brand", DbTypes.Types.String).Value = entity.brand ?? "";
            db.AddParameter(cmd, "outlet", DbTypes.Types.String).Value = entity.outlet ?? "";
            db.AddParameter(cmd, "stock", DbTypes.Types.Long).Value = entity.stock;
            db.AddParameter(cmd, "reorderLevel", DbTypes.Types.Long).Value = entity.reorderLevel;
            db.AddParameter(cmd, "unitCost", DbTypes.Types.Decimal).Value = entity.unitCost;
            db.AddParameter(cmd, "sellPrice", DbTypes.Types.Decimal).Value = entity.sellPrice;
            db.AddParameter(cmd, "batch", DbTypes.Types.String).Value = entity.batch ?? "";
            db.AddParameter(cmd, "expiry", DbTypes.Types.Date).Value = entity.expiry ?? DateTime.UtcNow.Date;
            db.AddParameter(cmd, "vendorId", DbTypes.Types.Long).Value = entity.vendorId;
