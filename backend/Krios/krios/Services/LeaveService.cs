...
namespace Krios.Services.Krios
{
    public class LeaveService
    {
        ...
        public async Task<List<Leave>> Select(LeaveSelectReq req)
        {
        ...
        public async Task<Leave> Insert(Leave entity)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task InsertTransaction(IDb db, Leave entity)
        {
            const string query = @"
                INSERT INTO leaves (
        ...
        public async Task<bool> UpdateTransaction(IDb db, Leave entity)
        {
            const string query = @"
                UPDATE leaves SET
                    ""orgId"" = @orgId,
        ...
        public async Task<bool> Delete(LeaveDeleteReq req)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> DeleteTransaction(IDb db, LeaveDeleteReq req)
        {
            const string query = @"
                UPDATE leaves
                SET status = 'Inactive',
        ...
        private static Leave Map(DbDataReader reader)
        {
            return new Leave
            {
        ...
        private static void Bind(DbCommand cmd, IDb db, Leave entity, bool includeId)
        {
            if (includeId)
            ...
            db.AddParameter(cmd, "fromDate", DbTypes.Types.Date).Value = entity.fromDate ?? DateTime.UtcNow.Date;
            db.AddParameter(cmd, "toDate", DbTypes.Types.Date).Value = entity.toDate ?? DateTime.UtcNow.Date;
