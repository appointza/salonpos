...
namespace Krios.Services.Krios
{
    public class MembershipService
    {
        ...
        public async Task<List<Membership>> Select(MembershipSelectReq req)
        {
        ...
        public async Task<List<Membership>> SelectTransaction(IDb db, MembershipSelectReq req)
        {
            const string query = @"
                SELECT id, ""orgId"", ""locationId"", plan, ""startDate"", ""endDate"", used, status, createdby, createdon, updatedby, updatedon, ""planId"", ""customerId""
                FROM memberships
            ";
        ...
        public async Task<Membership> Insert(Membership entity)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task InsertTransaction(IDb db, Membership entity)
        {
            const string query = @"
                INSERT INTO memberships (
                    ""orgId"", ""locationId"", plan, ""startDate"", ""endDate"", used, status, createdby, createdon, updatedby, updatedon, ""planId"", ""customerId""
                )
                VALUES (
                    @orgId, @locationId, @plan, @startDate, @endDate, @used, @status, @createdby, @createdon, @updatedby, @updatedon, @planId, @customerId
                )
        ...
        public async Task<bool> UpdateTransaction(IDb db, Membership entity)
        {
            const string query = @"
        ...
        public async Task<bool> Delete(MembershipDeleteReq req)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> DeleteTransaction(IDb db, MembershipDeleteReq req)
        {
            const string query = @"
                UPDATE memberships
                SET status = 'Inactive',
        ...
        private static Membership Map(DbDataReader reader)
        {
        ...
        private static void Bind(DbCommand cmd, IDb db, Membership entity, bool includeId)
        {
            if (includeId)
            ...
            db.AddParameter(cmd, "plan", DbTypes.Types.String).Value = entity.plan ?? "";
            db.AddParameter(cmd, "startDate", DbTypes.Types.String).Value = entity.startDate ?? "";
            db.AddParameter(cmd, "endDate", DbTypes.Types.String).Value = entity.endDate ?? "";
            db.AddParameter(cmd, "used", DbTypes.Types.Long).Value = entity.used;
