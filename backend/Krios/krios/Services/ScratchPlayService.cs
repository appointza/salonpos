...
namespace Krios.Services.Krios
{
    public class ScratchPlayService
    {
        ...
        public async Task<List<ScratchPlay>> Select(ScratchPlaySelectReq req)
        {
        ...
        public async Task<List<ScratchPlay>> SelectTransaction(IDb db, ScratchPlaySelectReq req)
        {
            const string query = @"
                SELECT id, ""orgId"", ""locationId"", ""customerId"", ""programId"", ""prizeId"", ""rewardType"", ""rewardValue"", label, source, ""referenceId"", ""checkinId"", status, ""createdAt"", ""loyaltyTransactionId"", createdby, createdon, updatedby, updatedon
                FROM ""scratchPlays""
            ";
            ...
            var result = new List<ScratchPlay>();
        ...
        public async Task<ScratchPlay> Insert(ScratchPlay entity)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task InsertTransaction(IDb db, ScratchPlay entity)
        {
            const string query = @"
                INSERT INTO ""scratchPlays"" (
                    ""orgId"", ""locationId"", ""customerId"", ""programId"", ""prizeId"", ""rewardType"", ""rewardValue"", label, source, ""referenceId"", ""checkinId"", status, ""createdAt"", ""loyaltyTransactionId"", createdby, createdon, updatedby, updatedon
                )
                VALUES (
                    @orgId, @locationId, @customerId, @programId, @prizeId, @rewardType, @rewardValue, @label, @source, @referenceId, @checkinId, @status, @createdAt, @loyaltyTransactionId, @createdby, @createdon, @updatedby, @updatedon
                )
        ...
        public async Task<ScratchPlay> Update(ScratchPlay entity)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> UpdateTransaction(IDb db, ScratchPlay entity)
        {
            const string query = @"
                UPDATE ""scratchPlays"" SET
                    ""orgId"" = @orgId,
        ...
        public async Task<bool> Delete(ScratchPlayDeleteReq req)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> DeleteTransaction(IDb db, ScratchPlayDeleteReq req)
        {
            const string query = @"
                UPDATE ""scratchPlays""
                SET status = 'Inactive',
        ...
        private static ScratchPlay Map(DbDataReader reader)
        {
            return new ScratchPlay
            {
                ...
                prizeId = ReadLong(reader, "prizeId"),
        ...
        private static void Bind(DbCommand cmd, IDb db, ScratchPlay entity, bool includeId)
        {
            if (includeId)
            ...
            db.AddParameter(cmd, "prizeId", DbTypes.Types.Long).Value = entity.prizeId;
