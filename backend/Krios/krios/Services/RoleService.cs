...
namespace Krios.Services.Krios
{
    public class RoleService
    {
        ...
        public async Task<List<Role>> Select(RoleSelectReq req)
        {
        ...
        public async Task<List<Role>> SelectTransaction(IDb db, RoleSelectReq req)
        {
            const string query = @"
                SELECT id, ""orgId"", ""locationId"", name, code, description, ""builtIn"", view, edit, status, createdby, createdon, updatedby, updatedon
                FROM roles
            ";
        ...
        public async Task InsertTransaction(IDb db, Role entity)
        {
            const string query = @"
                INSERT INTO roles (
                    ""orgId"", ""locationId"", name, code, description, ""builtIn"", view, edit, status, createdby, createdon, updatedby, updatedon
                )
                VALUES (
                    @orgId, @locationId, @name, @code, @description, @builtIn, @view, @edit, @status, @createdby, @createdon, @updatedby, @updatedon
                )
        ...
        public async Task<bool> UpdateTransaction(IDb db, Role entity)
        {
            const string query = @"
                UPDATE roles SET
                    ""orgId"" = @orgId,
        ...
        public async Task<bool> Delete(RoleDeleteReq req)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> DeleteTransaction(IDb db, RoleDeleteReq req)
        {
            const string query = @"
                UPDATE roles
                SET status = 'Inactive',
        ...
        private static Role Map(DbDataReader reader)
        {
            return new Role
            {
                ...
                builtIn = reader["builtIn"]?.ToString() ?? "",
        ...
        private static void Bind(DbCommand cmd, IDb db, Role entity, bool includeId)
        {
            if (includeId)
            ...
            db.AddParameter(cmd, "builtIn", DbTypes.Types.String).Value = entity.builtIn ?? "";
            db.AddParameter(cmd, "view", DbTypes.Types.String).Value = entity.view ?? "";
            db.AddParameter(cmd, "edit", DbTypes.Types.String).Value = entity.edit ?? "";
