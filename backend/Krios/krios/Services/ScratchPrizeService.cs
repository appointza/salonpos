...
namespace Krios.Services.Krios
{
    public class ScratchPrizeService
    {
        ...
        public async Task<List<ScratchPrize>> Select(ScratchPrizeSelectReq req)
        {
        ...
        public async Task<List<ScratchPrize>> SelectTransaction(IDb db, ScratchPrizeSelectReq req)
        {
            const string query = @"
            ...
            var result = new List<ScratchPrize>();
        ...
        public async Task<ScratchPrize> Insert(ScratchPrize entity)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task InsertTransaction(IDb db, ScratchPrize entity)
        {
            const string query = @"
                INSERT INTO ""scratchPrizes"" (
        ...
        public async Task<ScratchPrize> Update(ScratchPrize entity)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> UpdateTransaction(IDb db, ScratchPrize entity)
        {
            const string query = @"
                UPDATE ""scratchPrizes"" SET
                    ""orgId"" = @orgId,
        ...
        public async Task<bool> Delete(ScratchPrizeDeleteReq req)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> DeleteTransaction(IDb db, ScratchPrizeDeleteReq req)
        {
            const string query = @"DELETE FROM ""scratchPrizes"" WHERE id = @id";
        ...
        private static ScratchPrize Map(DbDataReader reader)
        {
            return new ScratchPrize
            {
        ...
        private static void Bind(DbCommand cmd, IDb db, ScratchPrize entity, bool includeId)
        {
            if (includeId)
