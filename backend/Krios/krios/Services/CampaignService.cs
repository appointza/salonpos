...
namespace Krios.Services.Krios
{
    public class CampaignService
    {
        ...
        public async Task<List<Campaign>> Select(CampaignSelectReq req)
        {
        ...
        public async Task<List<Campaign>> SelectTransaction(IDb db, CampaignSelectReq req)
        {
            const string query = @"
                SELECT id, ""orgId"", ""locationId"", name, channel, segment, audience, sent, opened, converted, budget, schedule, status, createdby, createdon, updatedby, updatedon
                FROM campaigns
            ";
        ...
        public async Task InsertTransaction(IDb db, Campaign entity)
        {
            const string query = @"
                INSERT INTO campaigns (
                    ""orgId"", ""locationId"", name, channel, segment, audience, sent, opened, converted, budget, schedule, status, createdby, createdon, updatedby, updatedon
                )
                VALUES (
                    @orgId, @locationId, @name, @channel, @segment, @audience, @sent, @opened, @converted, @budget, @schedule, @status, @createdby, @createdon, @updatedby, @updatedon
                )
        ...
        public async Task<bool> UpdateTransaction(IDb db, Campaign entity)
        {
            const string query = @"
        ...
        public async Task<bool> Delete(CampaignDeleteReq req)
        {
            using IDb db = await dbprovider.GetDb();
            await db.Connect();
        ...
        public async Task<bool> DeleteTransaction(IDb db, CampaignDeleteReq req)
        {
            const string query = @"
                UPDATE campaigns
                SET status = 'Inactive',
        ...
        private static Campaign Map(DbDataReader reader)
        {
            return new Campaign
            {
                ...
                audience = ReadLong(reader, "audience"),
                sent = ReadLong(reader, "sent"),
                opened = ReadLong(reader, "opened"),
                converted = ReadLong(reader, "converted"),
                budget = ReadDecimal(reader, "budget"),
        ...
        private static void Bind(DbCommand cmd, IDb db, Campaign entity, bool includeId)
        {
            if (includeId)
            ...
            db.AddParameter(cmd, "segment", DbTypes.Types.String).Value = entity.segment ?? "";
            db.AddParameter(cmd, "audience", DbTypes.Types.Long).Value = entity.audience;
            db.AddParameter(cmd, "sent", DbTypes.Types.Long).Value = entity.sent;
            db.AddParameter(cmd, "opened", DbTypes.Types.Long).Value = entity.opened;
            db.AddParameter(cmd, "converted", DbTypes.Types.Long).Value = entity.converted;
            db.AddParameter(cmd, "budget", DbTypes.Types.Decimal).Value = entity.budget;
            db.AddParameter(cmd, "schedule", DbTypes.Types.String).Value = entity.schedule ?? "";
